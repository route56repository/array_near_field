...
for j = 1:NR
        ...
        inc_D = (D)*floor(canvi_DQ);
        inc_DD = inc_D+D;
        AntenasRx(j,1) = sign(cos(inc_fase))*dRx;
        AntenasRx(j,2) = sign(sin(inc_fase))*dRx;
        inc_z = sqrt(inc_DD^2-(2*AntenasRx(j,1)^2));
        AntenasRx(j,3) = inc_z; 
 end
...
filename = strcat('O_MultiCurvado_txd2M64x64_dRx=3,7m_dmd16_Dgrupos100_400');
save(filename,'H');
